...
<%
	...
	*/
	...
	while(listRs.next()) {
		// DeptEmp 객체 생성 및 INNER JOIN 결과 처리를 위해 DeptEmp 안의 Employee, Departmet 객체 생성
		DeptEmp deptEmp = new DeptEmp();
		deptEmp.emp = new Employee();
		deptEmp.dept = new Department();
		
		// list 결과 ArrayList 에 저장
		deptEmp.emp.empNo = listRs.getInt("empNo");
		deptEmp.emp.firstName = listRs.getString("firstName");
		deptEmp.emp.lastName = listRs.getString("lastName");
		deptEmp.dept.deptName = listRs.getString("deptName");
		deptEmp.fromDate = listRs.getString("fromDate");
		deptEmp.toDate = listRs.getString("toDate");
		...
		<div class = "container">
			...
			<div class = "text-center">
				...
				%>
						...
						<a href = "<%=request.getContextPath() %>/deptEmp/deptEmpList.jsp?currentPage=1&deptEmpWord=<%=deptEmpWord %>">처음 </a>
						<%
							if(currentPage > 1) {
						%>
								<a href = "<%=request.getContextPath() %>/deptEmp/deptEmpList.jsp?currentPage=<%=currentPage - 1%>&deptEmpWord=<%=deptEmpWord %>">이전 </a>
						<%					
							}
						
							if(currentPage < lastPage) {
						%>
								<a href = "<%=request.getContextPath() %>/deptEmp/deptEmpList.jsp?currentPage=<%=currentPage + 1%>&deptEmpWord=<%=deptEmpWord %>">다음 </a>
						<%						
							}
						%>
						<a href = "<%=request.getContextPath() %>/deptEmp/deptEmpList.jsp?currentPage=<%=lastPage %>&deptEmpWord=<%=deptEmpWord %>">마지막 </a>
				<%
					}
				%>
			</div>
